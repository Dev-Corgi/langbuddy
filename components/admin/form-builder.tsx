...
export function FormBuilder({
  ...
  const withEffectiveTitles = (data: FormData): FormData => {
    if (!isAutoTitle || !autoTitles) return data
    return {
      ...data,
      title: autoTitles.title,
      title_en: autoTitles.title_en,
    }
  }

  const emit = (next: FormData) => {
    onChange(withEffectiveTitles(next))
  }
  ...
  const addQuestion = () => {
    ...
    emit({ ...value, questions: [...questions, newQuestion] })
  }

  const removeQuestion = (qId: string) => {
    if (questions.length === 1) return
    emit({ ...value, questions: questions.filter((q) => q.id !== qId) })
  }
  ...
  const updateOption = (qId: string, optIdx: number, val: string, isEn = false) => {
    emit({
      ...value,
      questions: questions.map((q) => {
            ...
            <>
              <div className="space-y-3">
                ...
                <AutoResizeTextarea
                  ...
                  value={value.title || ''}
                  onChange={(val) => emit({ ...value, title: val })}
              ...
              <div className="space-y-3">
                ...
                <AutoResizeTextarea
                  ...
                  onChange={(val) => emit({ ...value, title_en: val })}
        ...
        <div className="space-y-6">
          <div className="space-y-3">
            ...
            <AutoResizeTextarea
              ...
              onChange={(val) => emit({ ...value, description: val })}
          ...
          <div className="space-y-3">
            ...
            <AutoResizeTextarea
              ...
              onChange={(val) => emit({ ...value, description_en: val })}
              minHeight="min-h-[100px]"
        ...
        <div className="pt-6 border-t border-border">
          <div className="space-y-4">
            <Label className="text-sm font-bold text-muted-foreground flex items-center gap-2">
              ...
              실시간 데이터 연동 (Webhook)
              <span className="text-[10px] bg-primary/10 text-primary px-2 py-1 rounded-full font-medium ml-1">
                PRO
              </span>
            </Label>
            <Input
              ...
              onChange={(e) => emit({ ...value, webhook_url: e.target.value })}
                ...
                <CardContent className="p-4 md:p-10 space-y-6 md:space-y-8">
                  <div className="flex items-center justify-between gap-4 pb-4 border-b border-border/50">
                    <div className="flex items-center gap-2 md:gap-4">
                      ...
                      >
                        ...
                        <SelectContent className="rounded-xl border-border shadow-2xl">
                          <SelectItem value="text" className="font-bold py-2">
                            단답형
                          </SelectItem>
                          <SelectItem value="textarea" className="font-bold py-2">
                            장문형
                          </SelectItem>
                          <SelectItem value="radio" className="font-bold py-2">
                            객관식
                          </SelectItem>
                          <SelectItem value="checkbox" className="font-bold py-2">
                            체크박스
                          </SelectItem>
                          <SelectItem value="select" className="font-bold py-2">
                            드롭다운
                          </SelectItem>
                        </SelectContent>
                      </Select>
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      >
                        <Label className="text-[10px] md:text-xs font-black text-muted-foreground uppercase tracking-tight">
                          QR
                        </Label>
                        <input
                          type="checkbox"
                      ...
                      <div className="space-y-4 md:space-y-6">
                        ...
                        <div className="space-y-3 md:space-y-4">
                          {q.options.map((opt, optIdx) => (
                            <div
                              key={`${q.id}-opt-${optIdx}`}
                              className="flex items-center gap-2 md:gap-4 bg-muted/50 p-2 md:p-4 rounded-[20px] md:rounded-3xl border border-border hover:border-primary/30 transition-all group/opt"
                            >
